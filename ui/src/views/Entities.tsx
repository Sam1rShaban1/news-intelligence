...
function EntityDrawer({ entity, onClose }: { entity: any; onClose: () => void }) {
  ...
  const [tab, setTab] = useState<'articles' | 'relations'>('articles')

  useEffect(() => {
    if (!entity) return
    setArticles(null)
    setRelations(null)
    api.entityArticles(entity.id, 12)
      .then((r: any) => setArticles(r.articles ?? []))
      .catch(() => setArticles([]))
    api.entityRelations(entity.id, 20)
      .then((r: any) => setRelations(r.relationships ?? []))
      .catch(() => setRelations([]))
  }, [entity?.id])
  ...
  return (
    ...
    >
      <div
        style={{ width: '100%', maxWidth: 440, height: '100%', background: '#f5f5f0', borderLeft: '2px solid #0a0a0a', boxShadow: '-6px 0 0 #0a0a0a', overflowY: 'auto', display: 'flex', flexDirection: 'column' }}
      ...
      >
        <div style={{ padding: '14px 16px', borderBottom: '2px solid #0a0a0a', display: 'flex', alignItems: 'flex-start', gap: 10 }}>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 8, letterSpacing: '0.15em', color: '#555550', marginBottom: 4 }}>
              {LABEL_GLYPHS[entity.label] ?? '○'} {entity.label} / ENTITY
            </div>
            <div style={{ fontSize: 18, fontWeight: 700, lineHeight: 1.2 }}>{entity.text}</div>
            {entity.aliases?.length > 0 && (
              <div style={{ fontSize: 9, color: '#555550', marginTop: 3 }}>AKA: {entity.aliases.join(', ')}</div>
            )}
            <div style={{ fontSize: 9, marginTop: 6 }}>
              <span style={{ fontVariantNumeric: 'tabular-nums', fontWeight: 700 }}>{entity.mention_count}</span>
              <span style={{ color: '#555550', marginLeft: 4 }}>MENTIONS</span>
            </div>
          </div>
        ...
        <div style={{ display: 'flex', borderBottom: '1px solid #0a0a0a' }}>
          {(['articles', 'relations'] as const).map((t, i) => (
            <button key={t} onClick={() => setTab(t)}
              style={{ flex: 1, padding: '8px 0', border: 'none', borderRight: t === 'articles' ? '1px solid #0a0a0a' : 'none', background: tab === t ? '#0a0a0a' : 'transparent', color: tab === t ? '#f5f5f0' : '#0a0a0a', fontSize: 9, letterSpacing: '0.12em', fontFamily: 'Space Mono, monospace', fontWeight: 700, cursor: 'pointer' }}>
              {t.toUpperCase()}
            </button>
          ))}
        </div>
        ...
        <div style={{ flex: 1 }}>
          {tab === 'articles' ? (
            articles === null ? <LoadingDots /> :
            articles.length === 0 ? <EmptyState msg="NO ARTICLES FOUND" /> :
            articles.map((a, i) => (
              <div key={a.id} style={{ padding: '10px 16px', borderBottom: '1px solid #d4d4cc' }}>
            ...
            relations === null ? <LoadingDots /> :
            relations.length === 0 ? <EmptyState msg="NO RELATIONS FOUND" /> :
            relations.map((r, i) => (
              <div key={i} style={{ padding: '10px 16px', borderBottom: '1px solid #d4d4cc' }}>
                <div style={{ fontSize: 10, display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                  <span style={{ fontWeight: 700 }}>{r.subject}</span>
                  <span style={{ color: '#555550', fontSize: 8, padding: '1px 6px', border: '1px dashed #555550' }}>{r.predicate}</span>
                  <span style={{ fontWeight: 700 }}>{r.object}</span>
                </div>
                {r.confidence != null && (
                  <div style={{ fontSize: 8, color: '#555550', marginTop: 3 }}>CONF: {(r.confidence * 100).toFixed(0)}%</div>
                )}
              </div>
            ))
          )}
